...
import {ReactComponent as RatingPlant} from '../../../../../assets/images/icons/rating-plant.svg';
import { AiTwotoneStar,AiOutlineStar } from "react-icons/ai";
import { useState } from 'react';

const PopUp = ({forgot,open,setOpen,onClick,passwordChanged,navigate,rating}) => {
    ...
    <>
                <Modal
                    ...
                    <Grid container justifyContent={'center'} alignItems='center' style={{height:'100vh'}}   >
                      <Grid item p={2} lg={rating?8:3} sm={rating?10:4} md={rating?10:4} xs={rating?10:8} xl={rating?8:3} direction='column' alignItems='center'   sx={{ display:'flex' ,minHeight:'300px',height:rating?"90vh":"300px", backgroundColor:'#1E1E1E',border:passwordChanged ?'0.5px solid #14A384':'none',borderRadius:5}} >
                      <div style={{display:'inline-flex',flexDirection:'column',justifyContent:'space-between',height:'100%',width:'100%',padding:'10px'}}>
                          ...
                          )}
                      {
                        rating && (
                          ...
                          <div style={{display:'flex',alignItems:'center',width:'100%',justifyContent:'space-between'}}>
                            <h2 style={{fontWeight:'normal'}}> New Unread Reviews: 2</h2>
                            <button
                                className="btn btn-solid"
                                style={{backgroundColor:'#4180FE',borderRadius:'5px',fontSize:'12px'}}
                                onClick={() => {
                                  console.log('press')
                          ...
                          {/* // Ratings Body */}
                          <div
                          style={{height:'100%',width:'100%',display:'flex',flexDirection:'column',justifyContent:'space-between',overflowY:'scroll'}}
                          className='ratingPopup'
                          >

                           {ratingData.map(item=>( 
                           <div style={{height:'40vh',width:'100%',display:'flex',flexDirection:'column',justifyContent:'space-evenly',gap:'10px',marginTop:'10px'}}>
                            <div style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                              ...
                              <label
                              style={{color:'#E4201E',fontSize:'14px',cursor:'pointer'}}
                              >
                               <u>Delete</u>
                              </label>
                            </div>
                            <span style={{fontSize:'12px',color:'#707070'}}>{item.body} </span>
